...
export default function galleryPage() {
  ...
  return (
    <div className='flex flex-col items-center justify-center'>
      <Card className='max-w-[1200px] w-full min-h-32 mt-12 h-[calc(100vh-64px-6rem)] max-h-[1200px] flex flex-col'>
        <div className='w-full flex flex-row gap-3 max-h-[960px] h-4/5'>
          ...
          <div className='w-[calc(25%-0.6rem)] overflow-auto flex flex-col gap-2 items-center'>
            <div />
            {images.map((image, index) => (
              <Card
                isPressable
                key={index}
                className={`${currentImage === index ? 'outline-2 outline-[#777777]' : 'outline-0'} aspect-square bg-opacity-15 rounded-lg w-[256px] min-w-[256px] h-[256px] min-h-[256px] overflow-hidden bg-white flex relative`}
              >
                <Image
                  src={image.image}
                  alt='gallery-image'
