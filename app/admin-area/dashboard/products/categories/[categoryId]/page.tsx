    ...
    <div className='flex align-center justify-center'>
      {category && !category.loading ? (
        <div className='flex flex-col'>
          <Card className='max-w-[450px] w-full mt-4'>
            ...
            <CardBody className='bg-white bg-opacity-5 flex flex-row gap-2'>
              <Button
                ...
                onClick={editMode ? () => handleReject() : router.back}
              >
          ...
          <Button className='mt-4 w-full' color='danger' variant='flat' onClick={handleDelete}>
            Usuń kategorię
          </Button>
        </div>
          ...
          {(onClose) => (
            <>
              ...
              <CardBody className='flex flex-col gap-2'>
                <div>
                  Jesteś pewien, że chcesz usunąć kategorię <strong>{category.name}</strong>? Tej operacji nie można
                  cofnąć.
                </div>
                <div className='flex flex-row gap-2'>
                  <Button className='w-full' onClick={onClose} color='default'>
                    Anuluj
                  </Button>
                  <Button
                    className='w-full'
